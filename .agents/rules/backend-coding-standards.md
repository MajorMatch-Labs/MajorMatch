...
1. **Môi trường & Ngôn ngữ**:
   * Python 3.10.11 trong virtual environment `backend-hpc/.venv` (quản lý bởi `uv`).
...
3. **Quản lý checkpoint & Artifacts**:
   * Model và Scaler sau khi huấn luyện được serialize bằng `joblib` và lưu trữ tại `backend-hpc/ml/models/`.
