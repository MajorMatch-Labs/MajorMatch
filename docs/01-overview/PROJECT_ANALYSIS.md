...
* Kết hợp mô hình **Public Cloud** và **Private HPC Node**:
  ...
  * **Private Compute Node (Laptop Legion i9/RTX 4060):** Toàn bộ tệp PDF nhạy cảm, quá trình bóc tách điểm số, lưu trữ vector và suy luận LLM (Qwen 2.5 7B qua Ollama) được thực thi hoàn toàn khép kín trong môi trường máy chủ nội bộ.
...
* Tận dụng thiết bị biên thông minh (Vsmart Joy 3 làm Edge Gateway) và GPU cục bộ mạnh mẽ giúp hệ thống vận hành hoàn toàn không phát sinh chi phí duy trì máy chủ GPU đắt đỏ trên AWS/GCP (thường từ $500 - $1,500/tháng cho cụm GPU suy luận).
...
└───────────────────────────────────────────┬────────────────────────────────────────────┘
                                            ...
                                            ▼
┌────────────────────────────────────────────────────────────────────────────────────────┐
│ TẦNG 2: EDGE CONTROL PLANE & GATEWAY (Thiết bị biên: Vsmart Joy 3 - Ubuntu Termux)     │
│ - Phần cứng: Snapdragon 632, 3GB RAM, Ubuntu Rootfs qua PRoot                          │
...
└───────────────────────────────────────────┬────────────────────────────────────────────┘
                                            ...
                                            ▼
┌────────────────────────────────────────────────────────────────────────────────────────┐
│ TẦNG 3: PRIVATE HPC COMPUTE NODE (Trạm tính toán: Legion i9-13900HX / RTX 4060 8GB)    │
...
### 2.2. Phân tích chi tiết Tầng 2: Edge Gateway & Control Plane (Vsmart Joy 3)
* **Bản chất kiến trúc:** Sử dụng một thiết bị di động vật lý độc lập (ARM64 Architecture) cài đặt môi trường Linux hoàn chỉnh (Ubuntu qua Termux).
* **Lợi ích chiến lược:**
  * **Tách biệt mặt phẳng điều khiển (Control Plane Isolation):** Máy chủ tính toán Legion không cần mở bất kỳ cổng kết nối nào ra ngoài Internet, cũng không cần chạy daemon Cloudflare Tunnel. Toàn bộ áp lực kết nối mạng bên ngoài do Joy 3 tiếp nhận và hấp thụ.
  * **Bảo vệ phần cứng GPU (Backpressure Protection):** Mô hình AI chạy trên GPU có giới hạn về số lượng yêu cầu xử lý đồng thời. Joy 3 đóng vai trò "van điều tiết" với Nginx Rate Limiting: Chặn đứng các đợt spam request hoặc tấn công DoS trước khi chúng có cơ hội chạm tới card RTX 4060.
  * **Tiết kiệm năng lượng:** Joy 3 có thể duy trì hoạt động 24/7 với mức tiêu thụ điện cực thấp (~3W-5W), sẵn sàng nhận diện tín hiệu đánh thức hoặc giữ kết nối đường hầm Cloudflare Tunnel ổn định.

### 2.3. Phân tích chi tiết Tầng 3: Private HPC Compute Node (Legion i9 / RTX 4060)
* **Bản chất kiến trúc:** Trạm tính toán nội bộ (On-premise Compute Node) sở hữu vi xử lý hiệu năng cao 24 nhân 32 luồng (Core i9-13900HX) và card đồ họa chuyên dụng NVIDIA GeForce RTX 4060 (8GB VRAM GDDR6).
* **Lợi ích chiến lược:**
...
flowchart TD
    ...
    subgraph EdgeGateway ["Tầng 2: Edge Gateway (Joy 3)"]
...
* **Quy trình kỹ thuật:**
  ...
  3. **Thực thi suy luận (Inference Phase với Qwen 2.5 7B):**
     Ollama điều phối tính toán trên GPU RTX 4060, sinh ra cấu trúc các Milestone:
     ```json
     {
...
* **Cơ chế kỹ thuật:**
  * **Cloudflare Tunnel (`cloudflared`):** Thiết lập một đường hầm mã hóa outbound từ Joy 3 ra Edge PoP của Cloudflare. Nhờ đó, mạng gia đình/nội bộ không cần mở cổng NAT (Port Forwarding), không cần IP tĩnh công cộng, loại bỏ nguy cơ bị scan cổng từ hacker.
  * **Nginx Token Bucket Rate Limiter:** Cấu hình bộ giới hạn lưu lượng nghiêm ngặt:
    ...
    ```
  * **Bộ nhớ đệm SQLite Response Cache:** Khi nhận yêu cầu phân tích một khung chương trình phổ quát hoặc yêu cầu trùng lặp mã băm hồ sơ, Gateway trích xuất dữ liệu từ file SQLite cục bộ trên Joy 3 và trả về kết quả trong thời gian $\le 15\text{ms}$.
...
| **Edge Gateway Node** | **Ubuntu trên Joy 3** | Raspberry Pi 4 / Cloud VPS | Tận dụng phần cứng điện thoại cũ tiết kiệm chi phí, tích hợp pin dự phòng chống sập nguồn khi mất điện. |
| **Reverse Proxy & Ingress**| **Nginx + Cloudflared** | Caddy / Traefik | Nginx cực kỳ nhẹ, tối ưu tài nguyên trên chip ARM di động; Cloudflared miễn phí, bảo mật không cần mở cổng modem. |
...
| **Mô hình AI Suy luận** | **Qwen 2.5 (7B) qua Ollama** | LLaMA 3 / Claude API | Qwen 2.5 7B có khả năng hiểu tiếng Việt vượt trội, bám sát chỉ dẫn cấu trúc JSON và chạy mượt mà trên 8GB VRAM GPU RTX 4060. |
...
#### Node 2: Vsmart Joy 3 (Edge Gateway)
* **CPU:** Qualcomm Snapdragon 632 (8 nhân Kryo 250, xung nhịp 1.8 GHz).
* **RAM:** 3GB LPDDR3 (Mức chiếm dụng hệ điều hành Ubuntu + Nginx + Cloudflared + SQLite chỉ rơi vào khoảng **650MB - 900MB RAM**, bảo đảm hoạt động đa nhiệm ổn định).
* **Bộ nhớ trong:** 32GB eMMC 5.1 (Dư dả lưu trữ SQLite cache và file logs).

#### Node 3: Laptop Legion Pro (Private HPC Compute)
* **CPU:** Intel Core i9-13900HX (24 nhân, 32 luồng, xung nhịp Turbo tối đa 5.4 GHz, 36MB Cache).
* **GPU:** NVIDIA GeForce RTX 4060 Laptop GPU (8GB VRAM GDDR6, TGP tối đa 140W).
* **RAM:** 16GB DDR5 4800MHz.
* **Hệ điều hành chủ:** Windows 11 Pro 64-bit.
* **Mức độ chiếm dụng tài nguyên dự kiến:**
  * Docker Engine & FastAPI Service: ~450MB RAM.
  * ChromaDB Vector Store: ~300MB RAM.
  * Ollama Core & Qwen 2.5 7B Q4_K_M: **~5.2GB VRAM** trên RTX 4060 và ~1.2GB System RAM.
  * Tổng dung lượng RAM hệ thống sử dụng: $\approx 8.5\text{GB} / 16\text{GB}$ (Đảm bảo an toàn tuyệt đối, không gây hiện tượng Out Of Memory hoặc phân trang ổ cứng).
...
* **Tốc độ sinh token của LLM (Qwen 2.5 7B):** $\mathbf{45 - 55\text{ tokens/giây}}$ (nhờ tăng tốc phần cứng qua CUDA 12 trên RTX 4060).
* **Độ trễ phản hồi token đầu tiên (TTFT - Time To First Token):** $\mathbf{< 700\text{ miligiây}}$.
* **Độ trễ trung chuyển qua Cloudflare Tunnel + Joy 3:** $\mathbf{40 - 80\text{ miligiây}}$ (với hạ tầng mạng cáp quang thông thường).
...
| **2** | Laptop Legion mất kết nối mạng LAN hoặc sleep | Cao | Toàn bộ yêu cầu phân tích AI bị ngắt quãng | Cài đặt chế độ High Performance không sleep khi cắm sạc; cấu hình Heartbeat API trên Joy 3 để cảnh báo nếu mất ping. |
| **3** | Người dùng gửi liên tục nhiều bảng điểm gây nghẽn GPU | Cao | Tràn VRAM GPU RTX 4060 dẫn đến crash container | Thiết lập hàng đợi tuần tự (Sequential Semaphore) trong FastAPI backend: Chỉ cho phép xử lý tối đa 1 tác vụ LLM đồng thời; các yêu cầu khác xếp hàng đợi tối đa 30s. |
