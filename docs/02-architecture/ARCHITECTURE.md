...
└──────────────────────────────────────────────────────────┬───────────────────────────────────────────────────────────┘
                                                           ...
                                                           ▼
┌──────────────────────────────────────────────────────────────────────────────────────────────────────────────────────┐
│ TẦNG 2: EDGE CONTROL PLANE & GATEWAY TIER (Thiết bị biên: Vsmart Joy 3 - Chạy Ubuntu qua Termux/PRoot)                │
│                                                                                                                      │
│  ┌──────────────────────────────────────────────────┐      ┌──────────────────────────────────────────────────────┐  │
...
└─────────────────────────────────────────────────────────────┼───────────────────────────────────────────────────────────┘
                                                              ...
                                                              ▼
┌──────────────────────────────────────────────────────────────────────────────────────────────────────────────────────┐
│ TẦNG 3: PRIVATE HPC COMPUTE NODE (Laptop Legion Pro: i9-13900HX, RTX 4060 8GB VRAM, 16GB RAM, Win 11 + Docker)       │
│                                                                                                                      │
│  ┌────────────────────────────────────────────────────────────────────────────────────────────────────────────────┐  │
...
* **Thiết bị triển khai:** Vsmart Joy 3 (SoC Snapdragon 632, 3GB RAM, 32GB Bộ nhớ trong).
* **Môi trường:** Ubuntu 22.04 LTS chạy trên nền Termux/PRoot.
* **Trách nhiệm kiến trúc:**
  1. **Ingress Controller thông qua Cloudflare Tunnel (`cloudflared`):**
     ...
     * Hoàn toàn không mở cổng inbound trên modem mạng gia đình (Không cần Port Forwarding hay DDNS), ẩn giấu toàn bộ dải IP mạng nội bộ trước các công cụ dò quét cổng (Port Scanner).
  2. **Kiểm soát tải & Chống quá tải GPU (Traffic Shaping & Rate Limiting):**
     ...
     * Áp dụng chính sách đệm `burst=5 nodelay`. Khi hàng đợi vượt quá 5 requests, Nginx lập tức trả về mã lỗi `HTTP 429 Too Many Requests`, bảo vệ cụm container backend và card GPU RTX 4060 không bao giờ bị rơi vào trạng thái tràn bộ nhớ (Out Of Memory - OOM).
...
* **Thiết bị triển khai:** Laptop Legion Pro (Intel Core i9-13900HX 24 nhân 32 luồng, NVIDIA GeForce RTX 4060 8GB GDDR6 VRAM, 16GB DDR5 RAM, Windows 11 Pro).
* **Môi trường ảo hóa:** Docker Desktop (WSL 2 Backend tích hợp NVIDIA Container Toolkit).
* **Trách nhiệm kiến trúc:**
...
| **Tầng 1 $\leftrightarrow$ Tầng 2 (Joy 3)** | Cloudflare Tunnel (mTLS) | Cloudflare Edge Token, X-Secret-Header | JSON REST, Multipart Form (PDF) |
| **Tầng 2 $\leftrightarrow$ Tầng 3 (Legion)**| HTTP LAN (Port 8000) | Giới hạn IP nội bộ (192.168.1.50) | JSON REST, SSE Stream |
